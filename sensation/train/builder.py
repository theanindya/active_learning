# This version of builder.py is specific to the SENSATION dataset. To use cityscape dataset, run the 2nd phase of the code only
import logging
import os
...
# This version of builder.py is specific to the Cityscapes dataset.
...
def get_loss(
    loss_func: LossFunction = LossFunction.DICE, mode: LossMode = LossMode.MULTICLASS
):
    loss_mode = None
    loss = None
    ...
    if loss_func == LossFunction.JACCARD:
        ...
        loss = smp.losses.TverskyLoss(mode=loss_mode, alpha=0.45, beta=0.4, gamma=1.0)
    elif loss_func == LossFunction.CROSS:
        ...
        weights = max_frequency / frequencies
        weights /= weights.sum() * len(weights)  # Normalize weights to sum to 1
...
def create_seg_model(
    ...
    if len(parts) == 3:
        ...
        error_msg = f"The model architecture is not correctly defined: {model_arc}"
        raise ValueError(error_msg)
...
def create_segmentation_head(model, model_name, num_classes):
    ...
    if model_name.startswith("pspnet"):
        return smp.base.SegmentationHead(
            in_channels=model.whole_model.arc.segmentation_head[0].in_channels,
            out_channels=num_classes,
            kernel_size=3,
            upsampling=model.whole_model.arc.segmentation_head[1].scale_factor,
        )
    else:
        return smp.base.SegmentationHead(
            in_channels=model.whole_model.arc.segmentation_head[0].in_channels,
