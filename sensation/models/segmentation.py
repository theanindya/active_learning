...
class SegModel(pl.LightningModule):
    ...
    def forward(self, x, features_only=False):
        if features_only:
            return self.whole_model.encoder(x)
    ...
    # Enable gradient checkpointing during training
        with torch.cuda.amp.autocast():  # Enable mixed precision
            ...
            outputs = self(images)
            loss = self.compute_loss(outputs, semantic_masks)
        images, semantic_masks = batch
        outputs = self(images)
        loss = self.compute_loss(outputs, semantic_masks)
    ...
    def validation_step(self, batch, batch_idx):
        images, semantic_masks = batch
        outputs = self(images)
        val_loss = self.compute_loss(outputs, semantic_masks)
    ...
    def test_step(self, batch, batch_idx):
        images, semantic_masks = batch
        outputs = self(images)
        test_loss = self.compute_loss(outputs, semantic_masks)
